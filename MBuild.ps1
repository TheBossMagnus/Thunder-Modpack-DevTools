...
foreach ($edition in $editions) {
	...
	if ((Test-Path -Path $oldPack) -and (Test-Path -Path $currentPack)) {
		...
		ModpackChangelogger.exe --old $oldPack --new $currentPack --file "$root\bin\$MCversion\$release\Changelog-$release+$loader-$MCversion.md" --config "$root\config.json"}
	else {

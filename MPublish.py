...
def publish(edition: Tuple[str, List[str]]) -> None:
    ...
    os.system("git push")
    subprocess.run(["gh", "release", "create", f"{version}+{mc_version}", "-R", f"{modpack_author}/{modpack_name}", "-d", "-t", f"Thunder {version} for {mc_version}", "--notes", "GitHub releases are not recommended for use. Please download the modpack from the Modrinth or curseforge pages instead."], check=False)
